import { AUTHOR_REGEX, CHANNEL_ID_REGEX, TITLE_REGEX, VIDEO_ID_REGEX } from "src/constants";
import { ThumbnailQuality, TranscriptLine, TranscriptResponse } from "src/types";
...
export class YouTubeService {
     ...
     */
    static getThumbnailUrl(videoId: string, quality: keyof ThumbnailQuality = 'maxres'): string {
     ...
     */
     async fetchTranscript(url: string, langCode = 'en'): Promise<TranscriptResponse> {
		try {
            ...
            const captions = await this.extractCaptions(videoPageBody, langCode);
	 ...
	 */
	private async extractCaptions(pageBody: string, langCode: string): Promise<string> {
		
		// Find the script containing player data
		const parsedBody = parse(pageBody);
		const playerScript = parsedBody.getElementsByTagName("script")
			.find(script => script.textContent.includes("var ytInitialPlayerResponse = {"));

		if (!playerScript) throw new Error('Failed to find player data');

		// Extract player response data from script content
		const start = playerScript.textContent.indexOf("var ytInitialPlayerResponse = ") + 30;
		...
		const captionTrack = langCode ? 
			captionTracks.find((track: any) => track.languageCode.includes(langCode)) || captionTracks[0] :
			captionTracks[0];

		if (!captionTrack) throw new Error('No captions available');

		// Format and fetch captions URL
		const captionsUrl = captionTrack.baseUrl.startsWith("https://") ? captionTrack.baseUrl : `https://www.youtube.com${captionTrack.baseUrl}`;
